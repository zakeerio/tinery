                                    ...
                                    <form action="" method="post " enctype="multipart/form-data">
                                        ...
                                        @if (Auth::guard('user')->user() && $usercheck)
                                            <form action="{{ route('profilepictureupdate') }}" method="POST" id="formprofilepictrueupdate" enctype="multipart/form-data">
                                                <label for="profileimg" class="position-absolute bottom-0 end-0 position-absolute p-up ">
                                                    <i class="fa-solid fs-4 fa-circle-plus"></i>
                                                </label>
                                                <div class="d-none"><input type="file" name="file" id="profileimg">
                                                </div>
                                            </form>
                                        @endif
                                    </form>
                                ...
                                @if (Auth::guard('user')->user() && $usercheck)
                                    ...
                                    <div class="accordion-item border-0 border-bottom">
                                        <h2 class="accordion-header" id="relevant_Tags">
                                            <button class="accordion-button accordionbtn px-1 border-bottom collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseRelevantTags" aria-expanded="false" aria-controls="collapseRelevantTags">
                                                <span class="fs-18-600">Relevant Tags</span>
                                            </button>
                                        </h2>
                                        <div id="collapseRelevantTags" class="accordion-collapse collapse" aria-labelledby="relevant_Tags" data-bs-parent="#accordionExample">
                                            <div class="accordion-body p-1">
                                                        ...
                                                        <div class="tags d-flex flex-wrap mt-3 mb-2 gap-1">
                                                            @if ($singletag)
                                                                @foreach ($singletag as $singletagitem)
                                                                    @if ($singletagitem)
                                                                        <a href="{{ url('/tags/' . $singletagitem->slug) }}" class="mb-1"><button class="foodie text-nowrap">{{ $singletagitem->name }} </button> </a>
                                                                    @endif
                                                                @endforeach
                                                            @endif
                                                                ...
                                                                @foreach ($singletag as $singletagitem)
                                                                    @if ($singletagitem)
                                                                        <a href="{{ url('/tags/' . $singletagitem->slug) }}">
                                                                            <button class="foodie text-nowrap">{{ $singletagitem->name }} </button>
                                                                        </a>
                                                                    @endif
                                                                @endforeach
                                                            @endif
                                        ...
                                        @if (Auth::guard('user')->user() && $usercheck)
                                            <ul class="nav nav-tabs profile-tabs d-flex" id="myTabs" role="tablist">
                                                <li class="nav-item" role="presentation">
                                                    <button id="tab1" class=" nav-link active btn-dark-r1  py-2 " type="button" role="tab" aria-controls="content1" aria-selected="true" data-bs-toggle="tab" data-bs-target="#content1"> My Itinerary List </button>
                                                </li>
                                                <li class="nav-item" role="presentation">
                                                    <button class="btn-dark-r1  py-2 nav-link" id="tab2" data-bs-toggle="tab" data-bs-target="#content2" type="button" role="tab" aria-controls="content2" aria-selected="false"><i class="fa-regular fa-heart px-2"></i>Saved Itineraries</button>
                                                </li>
                                            </ul>
                                            <ul class="nav nav-tabs d-none d-md-block ">
                                                <li class="nav-item " role="presentation">
                                                    <a class="btn btn-danger rounded-pill px-4 px-md-3 py-2 text-nowrap fs-16-300 text-white add-itinerary " href="{{ url('/create-itinerary') }}">+ Add Itinerary</a>
                                                </li>
                                            </ul>
                                        @else
                                                    ...
                                                    <div class="d-flex gap-3 justify-content-between ">
                                                        <div class=" d-flex w-100   gap-3">
                                                            @if (!empty($itinerary->seo_image))
                                                                <img src="{{ asset('frontend/itineraries/' . $itinerary->seo_image) }}" alt="" class="col-3 w-120i px-0">
                                                            @else
                                                                <img src="{{ asset('frontend/images/annie-spratt.jpg') }}" alt="" class="col-3 w-120i px-0">
                                                            @endif
                                                            <div class="w-100">
                                                                ...
                                                                <div class="d-flex w-100 justify-content-between  gap-2">
                                                                    ...
                                                                    <div class=" d-flex  gap-2 align-items-center ">
                                                                        @if ($usercheck)
                                                                            <a href="{{ url('/edit-itinerary/' . $itinerary->id) }}" class="">
                                                                                <img src="{{ asset('frontend/images/edit-btn.png') }}" class="add-size-img">
                                                                            </a>
                                                                            <a href="javascript:;" data-id="{{ $itinerary->id }}" data-href="{{ url('/delete-itinerary/' . $itinerary->id) }}" class="add-size-img align-items-center border d-flex justify-content-center rounded-pill text-decoration-none deleteItinerary">
                                                                                <i class="fa fa-trash icons-color"></i>
                                                                            </a>
                                                                        @endif
                                                                        <a href="{{ route('itinerary', ['slug' => $itinerary->slug]) }}" class="">
                                                                            <img src="{{ asset('frontend/images/view-arrow.svg') }}" class="add-size-img">
                                                                        </a>
                                                                    </div>
                                                    ...
                                                    <div class="d-flex gap-3 justify-content-between ">
                                                        @if (!empty($itinerary->seo_image))
                                                            <div class="col-3 w-120i position-relative">
                                                                <img src="{{ asset('frontend/itineraries/' . $itinerary->seo_image) }}" alt="" class="w-120i">
                                                                <img src="frontend/images/heart-red.svg" alt="" class=" position-absolute heart-position heart-size-red">
                                                            </div>
                                                        @else
                                                            <img src="{{ asset('frontend/images/annie-spratt.jpg') }}" alt="" class="col-3 w-120i">
                                                        @endif
